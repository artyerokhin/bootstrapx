...
def _jackknife(data, statistic, n_jobs=1):
    """Leave-one-out jackknife with optional parallelism via joblib."""
    n = data.shape[0]
    if n_jobs == 1:
        return np.array(
    ...
    return np.array(
        Parallel(n_jobs=n_jobs)(
            delayed(statistic)(np.concatenate([data[:i], data[i + 1:]]))
            for i in range(n)
        )
    )


def bca_interval(boot_stats, data, statistic, theta_hat, confidence_level=0.95, n_jobs=1):
    alpha = 1.0 - confidence_level
    ...
    p_low = _adj(sp_stats.norm.ppf(alpha / 2))
    p_high = _adj(sp_stats.norm.ppf(1 - alpha / 2))
...
def studentized_interval(data, statistic, theta_hat, boot_stats, boot_se, confidence_level=0.95):
    alpha = 1.0 - confidence_level
    mask = boot_se > 0
