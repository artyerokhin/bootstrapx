...
class TestCluster:
    def test_run(self, cluster_setup):
        ...
        r = bootstrap(data, np.mean, method="cluster", n_resamples=1000, random_state=1, backend="vanilla", cluster_ids=ids)
...
class TestStrata:
    def test_run(self, strata_setup):
        ...
        r = bootstrap(data, np.mean, method="strata", n_resamples=1000, random_state=2, backend="vanilla", strata_ids=ids)
        assert r.n_resamples == 1000
