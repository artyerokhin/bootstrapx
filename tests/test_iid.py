...
class TestBCa:
    def test_ci_covers(self, normal_data):
        r = bootstrap(
            normal_data, np.mean, method="bca", n_resamples=5000, random_state=42, backend="vanilla"
        )
        assert r.confidence_interval.low < 5.5 and r.confidence_interval.high > 4.5

    def test_reproducibility(self, normal_data):
        r1 = bootstrap(
            normal_data, np.mean, method="bca", n_resamples=999, random_state=123, backend="vanilla"
        )
        r2 = bootstrap(
            normal_data, np.mean, method="bca", n_resamples=999, random_state=123, backend="vanilla"
        )
