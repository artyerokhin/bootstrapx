...
class TestPercentileInterval:
    ...
    def test_99_wider(self):
        b = np.random.default_rng(0).normal(0, 1, 10000)
        assert (percentile_interval(b, 0.99).high - percentile_interval(b, 0.99).low) > (
            percentile_interval(b, 0.95).high - percentile_interval(b, 0.95).low
        )
...
class TestStudentized:
    def test_shape(self):
        ...
        ci = studentized_interval(
            ...
            np.mean,
            5.0,
            rng.normal(5, 0.5, 2000),
            np.abs(rng.normal(0.5, 0.1, 2000)),
        )
        assert ci.low < ci.high
